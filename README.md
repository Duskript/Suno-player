...
> **Current build:** `0.1.23-auth-refresh-flow` (versionCode 24) — builds with JDK 17+ and Android SDK platform 35 via the Gradle wrapper. Requires JDK 17+ (Android Gradle Plugin 8.7) and the corresponding SDK platform. Auth refresh flow batch: tapping **Done** after an in-app Suno login now captures the WebView cookie and automatically validates it against `playlist/me` — Settings shows "Captured, validating…" then "Valid — Suno playlist/me returned HTTP 200." with **no separate Test Connection tap** (the manual Test Connection button and the paste-a-cookie flow still work). Status distinguishes Missing / Valid / Expired or rejected / Configured not tested (with expiry countdown), and rejection (HTTP 401/403) or a missing cookie surfaces "Login to Suno" as the recovery action. The background download worker now runs a **pre-sync auth guard**: safe WebView-cookie adoption (the Batch C no-overwrite-older rule), a freshness check, and a live `playlist/me` validation whenever the stored cookie is missing, expired, or expires within 5 minutes — auto-sync is then skipped with a concise "Auto-sync skipped: login required" summary (no WorkManager retry spam), manual sync fails loudly with the same guidance, and library data / downloaded audio are never cleared. If a sync still hits 401/403 mid-run (playlist fetch or per-track download), the worker attempts **one** safe WebView adoption + validation retry before surfacing login required — never more than one retry per run. Suno server-side session JWTs cannot be extended or forged by this app; the only legitimate refresh path is a fresh authenticated in-app WebView session, and the app says so honestly. Playback continues to be designed for perpetual background/screen-off operation while the queue has playable items and the user has not paused/stopped. All existing flows (cookie dialog, add playlist, backup import/export via SAF, resume playback, queue, track details) are unchanged.
