...
> **Current build:** `0.1.20-playback-lifetime-hardening` (versionCode 21) — builds with JDK 17+ and Android SDK platform 35 via the Gradle wrapper. Requires JDK 17+ (Android Gradle Plugin 8.7) and the corresponding SDK platform. Playback lifetime hardening: Settings now has a **Player Diagnostics** card (current track, player state, isPlaying/playWhenReady, queue length/index, repeat/shuffle, position, background keep-alive status, last error) and a **Media Controls** status section (Android 13+ notification permission with an open-settings action, plus the Media3 playback notification channel state), and the playback service/engine/player log lifecycle events (service create/session/task-removed/destroy, player/session creation, state and isPlaying transitions, player errors) so screen-off/background failures can be proven and fixed. This batch is evidence/hardening only — the screen-off bug is **not** claimed fixed here; durable notification/lockscreen controls are the next batch. All existing flows (cookie dialog, add playlist, backup import/export via SAF, resume playback, queue, track details) are unchanged.
