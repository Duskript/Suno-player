...
- **Local playback** — powered by Android Media3 / ExoPlayer with play/pause, next, previous, and shuffle. Unplugging a wired headset or losing a Bluetooth audio route pauses playback instead of blasting through the speaker. The current track, queue, and position are saved while playing, so after an app restart the Library page offers a **Resume** card to pick up where you left off (always user-tapped, never automatic). MediaSession metadata now carries the creator as the album title, giving the lockscreen/notification a stable grouping label. Media button / lockscreen control: a single registered media button receiver (`SunoMediaButtonReceiver`, a Media3 `MediaButtonReceiver` subclass) routes wired-headset, Bluetooth, and lockscreen play/pause/next/previous/seek events into the shared MediaSessionService, and tapping the media notification opens the app. The home screen widget uses explicit app-private provider actions for previous/play-pause/next against the shared player.
...
> **Current build:** `0.1.28-control-surface-fixes` (versionCode 29) — fixes direct control surfaces plus permitted auto-start prompts/resume paths. The home screen widget now sends explicit app-private actions to `SunoPlaybackWidgetProvider` for previous/play-pause/next instead of broadcasting media-button intents, while preserving the single `SunoMediaButtonReceiver` for headset, Bluetooth, lockscreen, and notification media keys. Android Auto track taps now expand a selected playable track into its containing saved-playlist queue, excluding unplayable tracks and starting at the tapped track; playlist rows remain browsable-only. Android 13+ notification permission is requested automatically when the Activity starts, while the Settings fallback remains for denied/don't-ask-again cases. Car, Bluetooth, and wired route-connect broadcasts trigger best-effort auto-resume from the existing player queue or saved playback state when Android delivers the broadcast and allows the app process to run. `SunoPlaybackService` remains a Media3 `MediaLibraryService` serving one shared `MediaLibrarySession` (id `suno-local-playback`, same process-wide ExoPlayer as before), and browse callbacks stay read-only over the app-private `LibraryStore` with no network calls, library mutation, cookie/JWT logging, Cars App Library UI, or AAOS-only module in this pass.
