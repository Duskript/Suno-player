...
> **Current build:** `0.1.24-media-player-polish` (versionCode 25) — builds with JDK 17+ and Android SDK platform 35 via the Gradle wrapper. Requires JDK 17+ (Android Gradle Plugin 8.7) and the corresponding SDK platform. Queue/offline preference polish batch: playback now **prefers verified local files** — a track's `localPath` is used only when the file actually exists and is non-empty (checked at queue-build time), and is fed to the player as a proper local file `Uri` (`Uri.fromFile`) instead of a raw path string. When the local file is stale/missing/zero-byte but an `audioUrl` exists, playback falls back to streaming and logcat says so honestly ("Local file missing for … — falling back to network audioUrl"). When a track has a stale local path and no network fallback, it is excluded from the queue with a clear user-facing message — `Missing local audio for "<title>" — resync or re-download this playlist.` — instead of a mysterious player error, and playback errors on a vanished local file are worded the same way (auto-skip guard from earlier batches is unchanged). **Resume diagnostics are no longer silent**: tapping Resume explains exactly why when it cannot rebuild the saved session (saved track missing from the library, saved queue has no playable tracks, or the saved track is unplayable because its local file is missing with no `audioUrl`), and the Resume card shows the saved queue size, saved position, and an honest "saved track no longer in library" fallback. Playback still never auto-starts — Resume is always user-tapped. **External-control sync from Batch B is preserved**: the `onPlayWhenReadyChanged` / `onTimelineChanged` / `onPositionDiscontinuity` / `onAvailableCommandsChanged` / `onEvents` listener overrides and the mediaId → track fallback mapping keep lockscreen/notification/Bluetooth next/previous/play/pause synced with the in-app queue. No library records or downloaded audio are deleted, no Suno endpoints changed, and no cookies/JWTs are logged. All existing flows (cookie dialog, add playlist, backup import/export via SAF, resume playback, queue, track details, Settings diagnostics) are unchanged.
