...
> **Current build:** `0.1.25-notification-lockscreen-fix` (versionCode 26) — hotfix for missing notification/lockscreen controls. Root cause: the Activity-created MediaSession could play audio, but `SunoPlaybackService` never registered that shared session with `addSession()`, so Media3 had no service-owned session linked to a foreground media notification. The service now registers the session, binds Media3 default notification creation to the app playback channel (`suno_local_playback`), and Settings checks that real channel instead of the old default id. Queue/offline polish from v0.1.24 remains intact: verified local file URIs are preferred, stale local files fall back to `audioUrl` when available, missing local-only tracks show clear resync/re-download guidance, Resume diagnostics are visible, and external-control sync listeners/mediaId fallback are preserved. No library records/downloaded audio are deleted, no Suno endpoints changed, and no cookies/JWTs are logged.
