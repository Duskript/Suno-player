...
> **Current build:** `0.1.17-now-playing-controls` (versionCode 18) — builds with JDK 17+ and Android SDK platform 35 via the Gradle wrapper. Requires JDK 17+ (Android Gradle Plugin 8.7) and the corresponding SDK platform. Media button / lockscreen control support: the app now registers exactly one media button receiver (`SunoMediaButtonReceiver`, a Media3 `MediaButtonReceiver` subclass) so `MediaSessionCompat` no longer logs "Couldn't find a unique registered media button receiver", and wired-headset / Bluetooth / lockscreen play-pause, next, previous, and seek events route into the shared MediaSessionService. The media notification tap also opens the app.
