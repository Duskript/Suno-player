...
> **Current build:** `0.1.26-home-screen-widget` (versionCode 27) — adds a compact Android home screen playback widget built with classic `AppWidgetProvider` + `RemoteViews` (no Glance dependency): it shows the current track title and creator, offers previous / play-pause / next controls, and opens the player when the widget body is tapped. The widget renders from the same state sync that drives the UI, notification, and lockscreen (`LocalAudioPlayer.syncStateFromPlayer()` — track transitions, play/pause, and command availability, never the 500ms position tick), and its buttons broadcast `ACTION_MEDIA_BUTTON` key events to the existing single `SunoMediaButtonReceiver`, so they route through the same MediaSessionService path as headset/lockscreen keys — no parallel playback stack and no second media button receiver. The v0.1.25 notification/lockscreen fix remains intact (service-registered MediaSession on the `suno_local_playback` channel). No library records/downloaded audio are deleted, no Suno endpoints changed, and no cookies/JWTs are logged.
