...
> **Current build:** `0.1.27-android-auto-connection` (versionCode 28) — adds Android Auto phone-app media support on top of the v0.1.25 notification/lockscreen fix and the v0.1.26 home screen widget. `SunoPlaybackService` is now a Media3 `MediaLibraryService` serving a single shared `MediaLibrarySession` (id `suno-local-playback`, same process-wide ExoPlayer as before) with a shallow, driver-safe browse tree: **Suno Local → Playlists → saved playlists → playable tracks**. Track URIs reuse `PlaybackSource` semantics (verified local file first, streaming `audioUrl` fallback; tracks with neither are hidden), and browse callbacks are read-only over the app-private `LibraryStore` — no network calls, no library mutation, no cookie/JWT logging. Android Auto support is declared via the `com.google.android.gms.car.application` application metadata pointing at `res/xml/automotive_app_desc.xml` (`<uses name="media"/>`); the manifest also keeps the `androidx.media3.session.MediaSessionService` action so the single `SunoMediaButtonReceiver` (headset/lockscreen/widget keys) keeps resolving this service. No templated Cars App Library UI and no AAOS-only module in this pass — Android Auto renders its own driver-safe UI. The v0.1.26 widget, notification channel `suno_local_playback`, and service-registered session are unchanged; no library records/downloaded audio are deleted and no Suno endpoints changed.
