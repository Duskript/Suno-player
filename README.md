...
> **Current build:** `0.1.22-cookie-freshness` (versionCode 23) — builds with JDK 17+ and Android SDK platform 35 via the Gradle wrapper. Requires JDK 17+ (Android Gradle Plugin 8.7) and the corresponding SDK platform. Cookie freshness batch: the app now models stored-cookie freshness explicitly (`CookieFreshness`: has session, JWT `exp` epoch, seconds-until-expiry, expired, expires-within-window, human status label such as "Expires in ~12 min"), and WebView cookie adoption is safe — a stored `__session` is never overwritten by an older/equal (or not-provably-newer) WebView `__session`; adoption only happens when the stored cookie is missing, already expired, or the WebView session is strictly newer, and non-`__session` WebView cookies ride along when a newer session is adopted. The pre-sync refresh in the download worker, the cookie refresh worker, and the Settings WebView capture now log/emit only captured/saved flags, a safe reason, and expiry timestamps — never cookie or JWT values. Settings shows the parsed expiry countdown next to "Configured — not tested". This batch is pure cookie-freshness correctness; automatic capture-and-validate on Done, pre-sync validation, and the one-tap re-login recovery flow are still in progress (next release). Playback continues to be designed for perpetual background/screen-off operation while the queue has playable items and the user has not paused/stopped. All existing flows (cookie dialog, add playlist, backup import/export via SAF, resume playback, queue, track details) are unchanged.
